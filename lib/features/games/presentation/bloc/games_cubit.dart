...
final class GamesCubit extends Cubit<GamesState> {
  ...
  Future<void> startGame(String gameKey) async {
    ...
    // Only use words with short udmurt for compile game
  ...
  void _emitQuestion({
    ...
    if (index >= questions.length) {
      emit(QuizComplete(score: score, total: questions.length, gameKey: gameKey));
      return;
    }
  ...
  Future<void> answerQuestion(String answer) async {
    ...
    await _saveResult(SaveResultParams(gameKey: state.gameKey, isCorrect: isCorrect));
    emit(QuizAnswered(
      isCorrect: isCorrect,
      previous: state,
  ...
  List<String> _buildOptions(
    ...
    if (gameKey == 'truefalse') {
      ...
      if (showCorrect) {
        ...
        final others = pool.where((w) => w.id != question.id).toList()..shuffle(_rng);
    ...
    final distractors = pool
        .where((w) => w.id != question.id)
        .toList()
      ..shuffle(_rng);
    final options = [question.russian, ...distractors.take(3).map((e) => e.russian)]
      ..shuffle(_rng);
    return options;
  }
}
